...
with col1:
    if F1_mag != 0.0:
        ax.arrow(0.0,0.0,0.0,F1x,width=width1,color='purple',length_includes_head=True,animated=True,head_width=head_w)
       # st.write(" $ \overrightarrow{F_1} = $ ","{:.4f}".format(F1x),"$ i $")
    if F2_mag !=0.0 and F2_theta != 0.0:    
        ax.arrow(F2_theta*np.pi/180.0,0.0,0.0,F2_mag,width=width2,color='red',length_includes_head=True,animated=True,head_width=head_w)
       # st.write(" $ \overrightarrow{F_2} = $ ","{:.4f}".format(F2x),"$ i $"," + ","{:.4f}".format(F2y),"$ j $")
    if F3_mag !=0.0 and F3_theta != 0.0:
        ax.arrow(F3_theta*np.pi/180.0,0.0,0.0,F3_mag,width=width3,color='blue',length_includes_head=True,animated=True,head_width=head_w)
        ax.arrow(FR_theta,0.0,0.0,FR_mag,width=width4,color='darkorange',length_includes_head=True,animated=True,head_width=head_w) 
